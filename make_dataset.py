...
def main(root: str, train_size: int, val_size: int, test_size: int,
         save_to: str = '.', min_width: int = 224, min_height: int = 224,
    ...
    # collect dataset
    for input_fnames, subset, n in zip((train_files, val_files, test_files),
                                       ('train', 'val', 'test'),
                                       (train_size, val_size, test_size)):
        input_files = [os.path.join(root, 'Data', 'CLS-LOC', subset, fname)
        ...
        copy_images(input_files, output_dir, n, min_width, min_height,
                    display_progress=display_progress, resize=resize)
...
def copy_images(input_files: list[str], output_dir: str, n: int,
                min_width: int = 224, min_height: int = 224,
                display_progress: bool = True, resize: bool = False):
    """
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train-size', type=int, help='train dataset size',
                        default=100000)
    parser.add_argument('--val-size', type=int, help='validation dataset size',
                        default=5000)
    parser.add_argument('--test-size', type=int, help='test dataset size',
                        default=10000)
    parser.add_argument('--save-to', type=str, help='output directory',
                        default='./data')
    ...
    parser.add_argument('--min-height', type=int, help='minimum image height',
                        default=224)
    parser.add_argument('--hide-progress', dest='show_progress',
                        action='store_false',
    ...
    random.seed(42)
    main(root=args.root, train_size=args.train_size, val_size=args.val_size,
         test_size=args.test_size, save_to=args.save_to,
