...
def main():
    ...
    train_dataset = ColorizationFolderDataset(
        folder="data/train", transforms=tv.transforms.RandomCrop(224)
    )
    ...
    while True:
        ...
        for X, Y in train_dataloader:
            ...
            if num_iter % eval_every == 0:
                ...
                val_losses.append(
                    evaluate(
                        model, val_dataloader, loss_fn, device, export_first, num_iter
                    )
                )
                ...
                if save_best and np.argmin(val_losses) == len(val_losses) - 1:
                    torch.save(
                        model.state_dict(), os.path.join(OUTPUT_DIR, "model.pth")
                    )
                model.train()
